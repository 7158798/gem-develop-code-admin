...
<html>
<head>
	<title>OTC交易记录列表</title>
	<%@ include file="/WEB-INF/views/include/head.jsp"%>
	<script type="text/javascript">
	  	$(function() {
	  		$('#dataGrid').datagrid({
				url : '${ctx}/bus/g_trade_record_otc_usdt/list',
				method: 'POST',
				title : '',
				iconCls : '',
				fit : true,
					...
					},
				{
					...
					formatter:function (value, row, index){
						...
						<shiro:hasPermission name="bus:g_trade_record_otc_usdt:view">
							a = '<li><a href="javaScript:void(0);" onclick="detail(\'详情\',\'${ctx}/bus/g_trade_record_otc_usdt/info?id='+row.id+'\',null,null)"><i class="fa fa-info-circle"></i>详情</a></li>';
						</shiro:hasPermission>
						<shiro:hasPermission name="bus:g_trade_record_otc_usdt:update">
							b = '<li><a href="javaScript:void(0);" onclick="update(\'修改\',\'${ctx}/bus/g_trade_record_otc_usdt/toUpdate?id='+row.id+'\',\'dataGrid\',null,null);"><i class="fa fa-edit"></i>修改</a></li>';
						</shiro:hasPermission>
						<shiro:hasPermission name="bus:g_trade_record_otc_usdt:delete">
  		...
  		function deleterow(id){
            createdialog('删除确认 ', '确定删除该记录吗 ?', '${ctx}/bus/g_trade_record_otc_usdt/delete?id='+id,'dataGrid');
        }
  		function bustripsearch(){
			...
			$('#dataGrid').datagrid({url:'${ctx}/bus/g_trade_record_otc_usdt/list',pageNumber:1});
		}
		function searchReset(){ 
			$("#toolDivNew").find(":input").val("");bustripsearch();
		}
		function toAdd(){
			add('新增','${ctx}/bus/g_trade_record_otc_usdt/toAdd','dataGrid',800,500);
		}
	</script>
  </head>
   <body class="psflow-panpel-body">
		...
		<div id="toolDiv" style="padding:2px 5px;">
			<div class="psflow-panpel-maintitle">
				<h2 class="psflow-panpel-title"><span>OTC交易记录列表</span></h2>
			</div>
			<div class="psflow-panpel-tooldiv">
				<div class="tooldiv" id="toolDivNew">
		   			<shiro:hasPermission name="bus:g_trade_record_otc_usdt:view">
		   			<span>订单编号:</span><input name = "g_trade_record_otc_usdt_vo.order_id" />
		   			<span>交易用户编号:</span><input name = "g_trade_record_otc_usdt_vo.uid"/>
		   			<span>原始用户编号:</span><input name = "g_trade_record_otc_usdt_vo.object_uid"/>
		   			<span>交易类型:</span>
		   			<select name = "g_trade_record_otc_usdt_vo.trade_type">
						<option value = "">--请选择--</option>
						<option value = "1">买入</option>
						<option value = "2">卖出</option>
		   			</select>
		   			<span>交易状态:</span>
		   			<select name = "g_trade_record_otc_usdt_vo.trade_status">
		   				<option value = "">--请选择</option>
	        		...
	        		<a href="javaScript:void(0);" class="u-btn u-btn-cgray" onclick="searchReset();" ><i class="fa fa-refresh"></i>重置</a>
		   			...
		   			<shiro:hasPermission name="bus:g_trade_record_otc_usdt:add">
