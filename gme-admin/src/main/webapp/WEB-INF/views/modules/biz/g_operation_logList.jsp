...
<head>
	...
	<script type="text/javascript">
	  	$(function() {
	  		$('#dataGrid').datagrid({
				url : '${ctx}/bus/g_operation_log/list',
				method: 'POST',
				title : '',
				iconCls : '',
				fit : true,
					...
					},
				{
					...
					formatter:function (value, row, index){
						...
						<shiro:hasPermission name="bus:g_operation_log:view">
							a = '<li><a href="javaScript:void(0);" onclick="detail(\'详情\',\'${ctx}/bus/g_operation_log/info?id='+row.id+'\',null,null)"><i class="fa fa-info-circle"></i>详情</a></li>';
						</shiro:hasPermission>
						<shiro:hasPermission name="bus:g_operation_log:update">
							b = '<li><a href="javaScript:void(0);" onclick="update(\'修改\',\'${ctx}/bus/g_operation_log/toUpdate?id='+row.id+'\',\'dataGrid\',null,null);"><i class="fa fa-edit"></i>修改</a></li>';
						</shiro:hasPermission>
						<shiro:hasPermission name="bus:g_operation_log:delete">
  		...
  		function deleterow(id){
            createdialog('删除确认 ', '确定删除该记录吗 ?', '${ctx}/bus/g_operation_log/delete?id='+id,'dataGrid');
        }
  		function bustripsearch(){
			...
			$('#dataGrid').datagrid({url:'${ctx}/bus/g_operation_log/list',pageNumber:1});
		}
		function searchReset(){ 
			$("#toolDivNew").find(":input").val("");bustripsearch();
		}
		function toAdd(){
			add('新增','${ctx}/bus/g_operation_log/toAdd','dataGrid',800,500);
		}
	</script>
  </head>
		...
		<div id="toolDiv" style="padding:2px 5px;">
			...
			<div class="psflow-panpel-tooldiv">
				<div class="tooldiv" id="toolDivNew">
					<span>用户编号:</span><input name="g_operation_log_vo.uid"/>
					<span>业务编号:</span><input name="g_operation_log_vo.biz_id"/>
					<span>业务类型:</span>
					<select name="g_operation_log_vo.biz_type">
					...
					<span>状态:</span>
					<select name="g_operation_log_vo.status">
						<option value="">--请选择--</option>
	        		...
	        		<a href="javaScript:void(0);" class="u-btn u-btn-cgray" onclick="searchReset();" ><i class="fa fa-refresh"></i>重置</a>
		   			...
		   			<shiro:hasPermission name="bus:g_operation_log:add">
