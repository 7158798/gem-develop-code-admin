...
<head>
	...
	<script type="text/javascript">
	  		...
	  		$('#dataGrid').datagrid({
					...
					},
				{
					...
					formatter:function (value, row, index){
						...
						<shiro:hasPermission name="biz:messageState:view">
							a = '<li><a href="javaScript:void(0);" onclick="detail(\'详情\',\'${ctx}/biz/messageState/info?id='+row.id+'\',null,null)"><i class="fa fa-info-circle"></i>详情</a></li>';
						</shiro:hasPermission>
						<shiro:hasPermission name="biz:messageState:update">
							b = '<li><a href="javaScript:void(0);" onclick="update(\'修改\',\'${ctx}/biz/messageState/toUpdate?id='+row.id+'\',\'dataGrid\',null,null);"><i class="fa fa-edit"></i>修改</a></li>';
						</shiro:hasPermission>
						<shiro:hasPermission name="biz:messageState:delete">
  		...
  		function deleterow(id){
            createdialog('删除确认 ', '确定删除该记录吗 ?', '${ctx}/biz/messageState/delete?id='+id,'dataGrid');
        }
  		function bustripsearch(){
			...
			$('#dataGrid').datagrid({url:'${ctx}/biz/messageState/list',pageNumber:1});
		}
		function searchReset(){ 
			$("#toolDivNew").find(":input").val("");bustripsearch();
		}
		function toAdd(){
			add('新增','${ctx}/biz/messageState/toAdd','dataGrid',800,500);
		}
	</script>
  </head>
		...
		<div id="toolDiv" style="padding:2px 5px;">
			...
			<div class="psflow-panpel-tooldiv">
				<div class="tooldiv" id="toolDivNew">
		   			<shiro:hasPermission name="biz:messageState:view">
		   			<span>业务编号:</span><input name = "messageStateVO.bizId"/>
		   			<span>用户编号:</span><input name = "messageStateVO.uid"/>
		   			<span>业务类型:</span>
		   			<select name = "messageStateVO.type">
	        		...
	        		<a href="javaScript:void(0);" class="u-btn u-btn-cgray" onclick="searchReset();" ><i class="fa fa-refresh"></i>重置</a>
		   			...
		   			<shiro:hasPermission name="biz:messageState:add">
