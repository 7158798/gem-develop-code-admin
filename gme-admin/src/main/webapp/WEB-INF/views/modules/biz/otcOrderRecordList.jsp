...
<head>
	...
	<script type="text/javascript">
	  	$(function() {
	  		$('#dataGrid').datagrid({
				url : '${ctx}/biz/otcOrderRecord/list',
				method: 'POST',
				title : '',
				iconCls : '',
				fit : true,
					...
					},
				{
					...
					formatter:function (value, row, index){
						...
						<shiro:hasPermission name="biz:otcOrderRecord:view">
							a = '<li><a href="javaScript:void(0);" onclick="detail(\'详情\',\'${ctx}/biz/otcOrderRecord/info?id='+row.entryOrderId+'\',null,null)"><i class="fa fa-info-circle"></i>详情</a></li>';
						</shiro:hasPermission>
						<shiro:hasPermission name="biz:otcOrderRecord:update">
							b = '<li><a href="javaScript:void(0);" onclick="update(\'修改\',\'${ctx}/biz/otcOrderRecord/toUpdate?id='+row.entryOrderId+'\',\'dataGrid\',null,null);"><i class="fa fa-edit"></i>修改</a></li>';
						</shiro:hasPermission>
						<shiro:hasPermission name="biz:otcOrderRecord:delete">
  		...
  		function deleterow(id){
            createdialog('删除确认 ', '确定删除该记录吗 ?', '${ctx}/biz/otcOrderRecord/delete?id='+id,'dataGrid');
        }
  		function bustripsearch(){
			...
			$('#dataGrid').datagrid({url:'${ctx}/biz/otcOrderRecord/list',pageNumber:1});
		}
		function searchReset(){ 
			$("#toolDivNew").find(":input").val("");bustripsearch();
		}
		function toAdd(){
			add('新增','${ctx}/biz/otcOrderRecord/toAdd','dataGrid',800,500);
		}
	</script>
  </head>
		...
		<div id="toolDiv" style="padding:2px 5px;">
			...
			<div class="psflow-panpel-tooldiv">
				<div class="tooldiv" id="toolDivNew">
		   			...
		   			<span>交易对象编号</span><input name = "otcOrderVO.objectUid">
		   			<span>业务类型:</span>
	        		...
	        		<a href="javaScript:void(0);" class="u-btn u-btn-cgray" onclick="searchReset();" ><i class="fa fa-refresh"></i>重置</a>
		   			...
		   			<shiro:hasPermission name="biz:otcOrderRecord:add">
