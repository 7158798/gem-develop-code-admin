...
<head>
	...
	<script type="text/javascript">
	  		...
	  		$('#dataGrid').datagrid({
					...
					},
				{
					...
					formatter:function (value, row, index){
						...
						<shiro:hasPermission name="biz:user:view">
							a = '<li><a href="javaScript:void(0);" onclick="detail(\'详情\',\'${ctx}/biz/user/info?id='+row.uid+'\',null,null)"><i class="fa fa-info-circle"></i>详情</a></li>';
						</shiro:hasPermission>
						<shiro:hasPermission name="biz:user:update">
							b = '<li><a href="javaScript:void(0);" onclick="update(\'修改\',\'${ctx}/biz/user/toUpdate?id='+row.uid+'\',\'dataGrid\',null,null);"><i class="fa fa-edit"></i>修改</a></li>';
						</shiro:hasPermission>
						<shiro:hasPermission name="biz:user:delete">
  		...
  		function deleterow(id){
            createdialog('删除确认 ', '确定删除该记录吗 ?', '${ctx}/biz/user/delete?id='+id,'dataGrid');
        }
  		function bustripsearch(){
			...
			$('#dataGrid').datagrid({url:'${ctx}/biz/user/list',pageNumber:1});
		}
		function searchReset(){ 
			$("#toolDivNew").find(":input").val("");bustripsearch();
		}
		function toAdd(){
			add('新增','${ctx}/biz/user/toAdd','dataGrid',800,500);
		}
	</script>
  </head>
		...
		<div id="toolDiv" style="padding:2px 5px;">
			...
			<div class="psflow-panpel-tooldiv">
				<div class="tooldiv" id="toolDivNew">
		   			<shiro:hasPermission name="biz:user:view">
		   			<span>用户名称:</span><input value="" name="userVO.name"/>
		   			<span>是否是商户:</span>
		   			<select name="userVO.isSupplier">
		   			...
		   			<span>是否开启二次验证:</span>
		   			<select name="userVO.isSecondVerify">
		   				<option value="">--请选择--</option>
	        		...
	        		<a href="javaScript:void(0);" class="u-btn u-btn-cgray" onclick="searchReset();" ><i class="fa fa-refresh"></i>重置</a>
		   			...
		   			<shiro:hasPermission name="biz:user:add">
