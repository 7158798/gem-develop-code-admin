...
<head>
	...
	<script type="text/javascript">
	  	$(function() {
	  		$('#dataGrid').datagrid({
				url : '${ctx}/biz/tradeRecordOtcUsdt/list',
				method: 'POST',
				title : '',
				iconCls : '',
				fit : true,
					...
					},
				{
					...
					formatter:function (value, row, index){
						...
						<shiro:hasPermission name="biz:tradeRecordOtcUsdt:view">
							a = '<li><a href="javaScript:void(0);" onclick="detail(\'详情\',\'${ctx}/biz/tradeRecordOtcUsdt/info?id='+row.id+'\',null,null)"><i class="fa fa-info-circle"></i>详情</a></li>';
						</shiro:hasPermission>
						<shiro:hasPermission name="biz:tradeRecordOtcUsdt:update">
							b = '<li><a href="javaScript:void(0);" onclick="update(\'修改\',\'${ctx}/biz/tradeRecordOtcUsdt/toUpdate?id='+row.id+'\',\'dataGrid\',null,null);"><i class="fa fa-edit"></i>修改</a></li>';
						</shiro:hasPermission>
						<shiro:hasPermission name="biz:tradeRecordOtcUsdt:delete">
  		...
  		function deleterow(id){
            createdialog('删除确认 ', '确定删除该记录吗 ?', '${ctx}/biz/tradeRecordOtcUsdt/delete?id='+id,'dataGrid');
        }
  		function bustripsearch(){
			...
			$('#dataGrid').datagrid({url:'${ctx}/biz/tradeRecordOtcUsdt/list',pageNumber:1});
		}
		function searchReset(){ 
			$("#toolDivNew").find(":input").val("");bustripsearch();
		}
		function toAdd(){
			add('新增','${ctx}/biz/tradeRecordOtcUsdt/toAdd','dataGrid',800,500);
		}
	</script>
  </head>
		...
		<div id="toolDiv" style="padding:2px 5px;">
			...
			<div class="psflow-panpel-tooldiv">
				<div class="tooldiv" id="toolDivNew">
		   			<shiro:hasPermission name="biz:tradeRecordOtcUsdt:view">
		   			<span>订单编号:</span><input name = "tradeRecordOtcUsdtVO.orderId" />
		   			<span>交易用户编号:</span><input name = "tradeRecordOtcUsdtVOUid"/>
		   			<span>原始用户编号:</span><input name = "tradeRecordOtcUsdtVO.objectUid"/>
		   			<span>交易类型:</span>
		   			<select name = "tradeRecordOtcUsdtVO.tradeType">
						<option value = "">--请选择--</option>
						<option value = "1">买入</option>
						<option value = "2">卖出</option>
		   			</select>
		   			<span>交易状态:</span>
		   			<select name = "tradeRecordOtcUsdtVO.tradeStatus">
		   				<option value = "">--请选择</option>
	        		...
	        		<a href="javaScript:void(0);" class="u-btn u-btn-cgray" onclick="searchReset();" ><i class="fa fa-refresh"></i>重置</a>
		   			...
		   			<shiro:hasPermission name="biz:tradeRecordOtcUsdt:add">
