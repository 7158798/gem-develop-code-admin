	...
	<head>
		...
		<script type="text/javascript">
							...
							},
							{
								...
								formatter:function (value, row, index){
									...
									<shiro:hasPermission name="sys:sysUser:view">
										a = '<li><a href="javaScript:void(0);" onclick="detail(\'详情\',\'${ctx}/sys/sysUser/info?id='+row.id+'\',null,null)"><i class="fa fa-info-circle"></i>详情</a></li>';
									</shiro:hasPermission>
									<shiro:hasPermission name="sys:sysUser:update">
										b = '<li><a href="javaScript:void(0);" onclick="update(\'修改\',\'${ctx}/sys/sysUser/toUpdate?id='+row.id+'\',\'dataGrid\',null,null);"><i class="fa fa-edit"></i>修改</a></li>';
									</shiro:hasPermission>
		  	...
		  	function deleterow(id){
	            createdialog('删除确认 ', '确定删除该记录吗 ?', '${ctx}/sys/sysUser/delete?id='+id,'dataGrid');
	        }
			...
			function rsetPass(id,us){ 
				var paramsData="sysUser.id="+id+"&sysUser.username="+us;
				$.ajax({
					async : false,
					cache : false,
					type : 'POST',
					data : paramsData,
					url : '${ctx}/sys/sysUser/rsetPass',
					error : function() {
					},
			...
			function bustripsearch(){
				...
				$('#dataGrid').datagrid({url:'${ctx}/sys/sysUser/list',pageNumber:1});
			}
			function searchReset(){ 
				$("#toolDivNew").find(":input").val("");bustripsearch();
			}
			function toAdd(){
				add('新增','${ctx}/sys/sysUser/toAdd','dataGrid',800,500);
			}
		</script>
	</head>
		...
		<div id="toolDiv" style="padding:2px 5px;">
			...
			<div class="psflow-panpel-tooldiv">
				<div class="tooldiv" id="toolDivNew">
					<span>姓名:</span><input type="text" name="sysUserVO.truename"/>
		   			<span>登录名:</span><input type="text" name="sysUserVO.userName"/>
		   			<span>手机号码:</span><input type="text" name="sysUserVO.phone"/>
		   			<shiro:hasPermission name="sys:sysUser:view">
